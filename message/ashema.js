...
import setting from '../lib/settings.json'
with {
    type: 'json'
};
import mess from '../lib/response.json'
with {
    type: 'json'
};
const {
            ...
            case prefix + 'kick': {
                ...
                if (mentionUser.length !== 0) {
                    karr.groupParticipantsUpdate(from, [mentionUser[0]], "remove")
                        .then(res => {
                            mentions(`Sukses mengeluarkan @${mentionUser[0].split("@")[0]}`, [mentionUser[0]], true)
                        })
                        .catch(() => reply(mess.error.api))
                } else if (isQuotedMsg) {
                    karr.groupParticipantsUpdate(from, [quotedMsg.sender], "remove")
                        .then(res => {
                            mentions(`Sukses mengeluarkan @${quotedMsg.sender.split("@")[0]}`, [quotedMsg.sender], true)
                        })
                        .catch(() => reply(mess.error.api))
                } else {
...
1:3 berarti Al Fatihah ayat 3`
 ...
 `)
        ...
        karr.sendMessage(msg.key.remoteJid, {
