...
export const serialize = (conn, msg) => {
  ...
  // Buka pembungkus ephemeral / viewOnce (bisa bertingkat) supaya konten asli
  // (text/caption) langsung terbaca. Tanpa ini, pesan sekali-lihat / ephemeral
  // membuat msg.type jadi nama pembungkusnya dan chat di handler jadi kosong.
  try {
    const wrappers = [
      "ephemeralMessage",
      "viewOnceMessage",
    ...
    let wrap = bail.getContentType && bail.getContentType(msg.message);
    while (wrap && wrappers.includes(wrap) && msg.message?.[wrap]?.message) {
      msg.message = msg.message[wrap].message;
      wrap = bail.getContentType && bail.getContentType(msg.message);
    }
  } catch {}

  try {
    // Pesan PERTAMA dari player baru di grup ikut membawa
    // `senderKeyDistributionMessage` (dan kadang `messageContextInfo`) sebagai
    // key pertama → kalau diambil Object.keys()[0], type jadi salah & chats
    // kosong sehingga bot tidak respon di percobaan pertama. getContentType
    // mengabaikan key non-konten itu dan mengambil tipe konten asli.
    const skip = ["senderKeyDistributionMessage", "messageContextInfo"];
    const berak =
      (bail.getContentType && bail.getContentType(msg.message)) ||
      Object.keys(msg.message).find((k) => !skip.includes(k)) ||
  ...
  try {
    ...
    msg.tipuAn.fromMe =
      msg.tipuAn.sender === conn.user.id.split(":")[0] + "@s.whatsapp.net";
    ...
    msg.tipuAn.key.device = bail.getDevice(
      msg.message[msg.type].contextInfo.stanzaId,
    );
  } catch {
    msg.tipuAn = null;
    msg.istipuAn = false;
  ...
  // Pastikan msg.body / msg.text / msg.chats SELALU terisi (string), mencakup
  // semua varian: conversation, caption media, extendedText, balasan
  // button/list/template. Jadi setiap chat yang masuk tidak pernah kosong.
  try {
    const c = msg.type ? msg.message?.[msg.type] : null;
    msg.body =
      (typeof c === "string" ? c : null) || // conversation
      ...
      c?.selectedButtonId ||
      c?.singleSelectReply?.selectedRowId ||
      c?.selectedId ||
      c?.title ||
      "";
  } catch {
...
export const randomNomor = (min, max = null) => {
  if (max !== null) {
    min = Math.ceil(min);
    max = Math.floor(max);
