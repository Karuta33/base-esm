  ...
  karr.multi = true
	...
	karr.spam = []
       ...
       return await karr.sendMessage(jid, {
        ...
        karr.reply = (from, content, msg) => karr.sendMessage(from, { text: content }, { quoted: msg })
	karr.allfitur = () => fs.readFileSync('./message/ashema.js').toString().split(`break`).length
	karr.ws.on('CB:call', async (json) => {
           const callerId = json.content[0].attrs['call-creator']
 karr.sendMessage(callerId, { text : '*Maaf bot tidak dapat menjawab call*' })
    await sleep(10000)
        })
		...
		await karr.relayMessage(jid, prepare.message, { messageId: prepare.key.id })
		return prepare
	 }
           ...
           await karr.sendMessage(jid, { sticker: { url: buffer }, ...options }, { quoted })
           .then( response => {
