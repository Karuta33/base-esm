      ...
      .then((data) => {
        ...
        for (let i of data) {
          var option = {
            ...
            surah: i.nama,
            latin: i.nama_latin,
            jumlah_ayat: i.jumlah_ayat,
            wahyu: i.tempat_turun,
            arti: i.arti,
            desk: i.deskripsi,
            audio: i.audio,
          };
...
export const getSurah = (surah, ayat) => {
  return new Promise((resolve, reject) => {
    if (ayat !== undefined) {
      fetchJson(`https://api.quran.sutanlab.id/surah/${surah}/${ayat}`)
        .then(async (data) => {
          if (data.code == 404)
            return resolve({
              status: 404,
              msg: data.message.replace("Ayah", "Ayat"),
            });
          let result = {
            status: 200,
            msg: "Succes!",
      ...
      fetchJson(`https://api.quran.sutanlab.id/surah/${surah}`)
        .then(async (data) => {
          if (data.code == 404)
            return resolve({ status: 404, msg: data.message });
          ...
          for (let i of data.data.verses) {
            var opt = {
              index_ayat: i.number.inSurah,
              ayat: i.text.arab,
              latin: i.text.transliteration.en,
              id: i.translation.id,
              en: i.translation.en,
              audio: i.audio.secondary[0],
            };
...
export const quranAudio = (no) => {
  return new Promise((resolve, reject) => {
    fetchJson("https://api.alquran.cloud/v1/ayah/" + no + "/ar.alafasy")
      .then((data) => {
        resolve(data);
      })
      .catch((e) => {
...
export const randomDoa = (no) => {
  return new Promise((resolve, reject) => {
    fetchJson(
      "https://raw.githubusercontent.com/Kieza17/doa-harian/main/doa-harian/doa-harian.json",
    )
      .then((data) => {
        resolve(data);
      })
