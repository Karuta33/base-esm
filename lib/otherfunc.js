...
// Mendefinisikan __filename untuk lingkungan ESM
...
// Internal function, tidak di-export kecuali dibutuhkan
...
export const updateProfilePicture = async (buffer) => {
  const jimp = await Jimp.read(buffer);
  const min = jimp.getWidth();
  const max = jimp.getHeight();
  const cropped = jimp.crop(0, 0, min, max);
...
export const parseMention2 = (text = "") => {
...
export const resizeImageFromUrl = async (
  ...
  try {
    console.log("Sedang mengunduh gambar...");
    ...
    console.log("Gambar berhasil diunduh. Mulai meresize...");
    await sharp(imageBuffer)
      .resize(width, height, {
        fit: "cover", // Pilihan: 'cover', 'contain', 'fill', 'inside', 'outside'
        position: "center",
      })
      .toFile(outputFilename); // Simpan hasil ke file

    console.log(
      `Selesai! Gambar berhasil di-resize dan disimpan sebagai: ${outputFilename}`,
    );
  } catch (error) {
...
export const reSize = async (buffer, ukur1, ukur2) => {
  try {
    ...
    throw new Error(`Gagal mengubah ukuran gambar: ${error.message}`);
  }
};
